...
# ================= HX711 类 =================
class HX711_gz:
    def __init__(self, dt_pin, sck_pin, offset=633400, scale=1000):
...
def camera_process(shared_weight):
    ...
    while True:
        ...
        cv2.putText(combined, f"Weight: {shared_weight.value:.2f}g", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255),
                    2)
        ...
        with open(CSV_FILE, mode="a", newline="") as f:
            csv.writer(f).writerow([now, f"{diameter_mm:.2f}", f"{shared_weight.value:.2f}", ocr_text])
